...
function nowTime(){
    ...
    return nowtime.getFullYear().toString() + pad2(nowtime.getMonth()+1) + pad2(nowtime.getDate()) + pad2(nowtime.getHours()) + pad2(nowtime.getMinutes()) + pad2(nowtime.getSeconds());
}
 ...
 * 验证还款设置 个人结算账户（本行）
*/
function checkAutoPayAcc(accNo){
	var accountNo = accNo;
	if(accountNo){
		if(!(new RegExp(/^[0-9]{1,19}$/).test(accountNo))){
			return false;
		}
		if(startsWith(accountNo,"1")||startsWith(accountNo,"6")){
			if(accountNo.length > 10){
				var num1 = accountNo.substring(8,10);
				if(parseInt(num1) >= 21 && parseInt(num1) <= 60){
					var ret = checkAccountNo(accountNo);
					if(ret){
						return true;
					}else{
		...
		}else if(startsWith(accountNo,"9")){
...
* 对还款设置中他行账户进行验证（跨行）
* */
function checkrRecAccount(accNo){
    ...
    var cardInfo = judgeAccRole2(debitCardNo);
...
function checkAccountNo(accountNo){
    var params = judgeAccRole2(accountNo);

    if(params.cardOwner == 'o'||params.cardOwner == "s"||params.cardOwner=="g" && params.cardType == "0"){
       return true; 
    }else{
       return false;
    }
...
// function isChiEngNum(val){
//     let reg = new RegExp("^[\u4e00-\u9fa5|A-Za-z|0-9]+$");
//     return reg.test(val);
// }